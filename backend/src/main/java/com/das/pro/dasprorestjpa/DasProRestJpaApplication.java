...
public class DasProRestJpaApplication {
	...
	// Native Entry point mapping command-line executions directly into embedded Tomcat lifecycle hooks
	public static void main(String[] args) {
		SpringApplication.run(DasProRestJpaApplication.class, args);
	}
	
	// Inject universal CORS policy filtering globally to avoid per-controller repetitions natively
	@Bean
	public WebMvcConfigurer corsConfigurer(){
		return new WebMvcConfigurer() {
			...
			public void addCorsMappings(CorsRegistry registry) {
				...
				registry.addMapping("/**")
						// Explicitly allow Create React App default port 3000 through the security gateway
